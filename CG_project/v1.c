...
void display(int state, int curX, int curY) {
	...
	if (mouse_left_button == 1 && state == GLUT_DOWN) { // 왼쪽 마우스 버튼이 눌려진 채 유지되는 상태
		switch (vertex_cnt)
		{
		case 0: // 첫 번째 정점일 경우
			// 점 그리기
			...
			// 마지막 정점으로 추가
			last_point_X = curX; last_point_Y = curY;
...
void mouse(int button, int state, int x, int y) { // 받아온 x, y는 왼쪽 위가 (0, 0) 원점
	...
	float curX, curY;
	convert_coordinate(x, y, &curX, &curY);
